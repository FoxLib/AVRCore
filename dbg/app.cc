...
void AVRDBG::create(int w, int h) {

    if (SDL_Init(SDL_INIT_EVERYTHING))
        exit(1);

    // Создать окно
    window = SDL_CreateWindow("AVR Debugger", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, w, h, SDL_WINDOW_SHOWN);
    if (window == NULL)
        exit(2);
...
int AVRDBG::poll() {
    ...
    // Крутить цикл до тех пор, пока что-нибудь не произойдет в нём
    while (!event_cause) {
...
// Запись пикселя RAW
void AVRDBG::pset(int x, int y, int cl) {
...
void AVRDBG::print16char(int col, int row, unsigned char ch, uint cl) {
...
// Печать строки
void AVRDBG::print(int col, int row, const char* s, uint cl) {
...
// Байт во флаги
void AVRDBG::byte_to_flag(unsigned char f) {
...
// Флаги в байты
unsigned char AVRDBG::flag_to_byte() {
...
unsigned int AVRDBG::neg(unsigned int n) {
    return n ^ 0xffff;
}

// Установка флагов
void AVRDBG::set_logic_flags(unsigned char r) {
...
void AVRDBG::set_subcarry_flag(int d, int r, int R, int carry) {
...
void AVRDBG::set_subtract_flag(int d, int r, int R) {
...
void AVRDBG::set_add_flag(int d, int r, int R, int carry) {
...
void AVRDBG::set_lsr_flag(int d, int r) {
...
void AVRDBG::set_adiw_flag(int a, int r) {
...
void AVRDBG::set_sbiw_flag(int a, int r) {
...
void AVRDBG::mask(const char* mask, int opcode) {
